...
const createSupplierHandler = async (req, res) => {
    ...
    try {
        ...
        await supplierService.createSupplier(name, numberPhone, address);
...
const readSupplierHandler = async (req, res) => {
    try {
        // Lưu kết quả vào biến 'results'
        const results = await supplierService.readSupplier();
...
const updateSupplierHandler = async (req, res) => {
    ...
    try {
        // Thực hiện câu lệnh SQL
        await supplierService.updateSupplier(name, numberPhone, address, id);
