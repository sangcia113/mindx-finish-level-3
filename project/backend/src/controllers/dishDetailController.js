const dishDetailService = require('../services/dishDetailService');

const readDishDetailHandler = async (req, res) => {
    try {
        // Lấy id cần truy vấn từ params
        const { id } = req.params;

        // Lưu kết quả vào biến 'results'
        const results = await dishDetailService.readDishDetail(id);
...
module.exports = { readDishDetailHandler };
